...
export default function WalletPage() {
  ...
  const wallet = useQuery({
    queryKey: ["wallet"],
    queryFn: async () => (await api.get<ApiResponse<Wallet>>("/wallet")).data.data,  
  });

  const transactions = useQuery({ 
    queryKey: ["wallet-transactions"],
    queryFn: async () => (await api.get<{ data: Transaction[] }>("/wallet/transactions")).data
  });
  
  const deposits = useQuery({ 
    queryKey: ["wallet-deposits"],
    queryFn: async () => (await api.get<Deposit[]>("/wallet/deposits")).data
  });
    ...
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Deposit {wallet.data.depositAddress.asset} on {wallet.data.depositAddress.network} and pay visa fees from your internal balance.
        </p>
      </div>
      <div className="grid gap-6 lg:grid-cols-2">
        <BalanceCard balance={wallet.data.balance} currency={wallet.data.currency} />
        <QRCodeCard address={wallet.data.depositAddress.address} />
      </div>
      <Card>
        <CardHeader>
            ...
            <div className="space-y-3">
              {deposits.data.map((deposit) => 
                <div key={deposit.id} className="flex items-center justify-between rounded-lg border p-3">
                  <div>
                    <p className="font-mono text-sm">{deposit.transactionHash.slice(0, 18)}...</p>
                    <p className="text-sm text-muted-foreground">{deposit.confirmations} confirmations</p>
                  </div>
            ...
            <div className="divide-y">
              {transactions.data.data.map((tx) => 
                <div key={tx.id} className="flex justify-between py-3">
                  ...
                  <p className={tx.amount >= 0 ? "text-emerald-600" : "text-destructive"}>{(tx.amount / 100).toFixed(2)}</p>
                </div>
              )}
            </div>
            :
