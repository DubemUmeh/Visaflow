...
function AdminSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <aside className={cn(
        'fixed top-0 left-0 h-full w-64 bg-gray-900 z-50 flex flex-col transition-transform duration-300',
        'lg:translate-x-0 lg:z-auto',
        open ? 'translate-x-0' : '-translate-x-full'
      )}>
        <div className="px-5 py-5 flex items-center justify-between border-b border-gray-700">
        ...
        <div className="px-3 py-4 border-t border-gray-700">
          <Link href="/dashboard" className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-400 hover:bg-gray-800 hover:text-white transition-all mb-1">
            <LayoutDashboard className="w-4 h-4" />
            Applicant View
          </Link>
          <button
            onClick={handleLogout}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-400 hover:bg-red-900/50 hover:text-red-400 transition-all"
          >
