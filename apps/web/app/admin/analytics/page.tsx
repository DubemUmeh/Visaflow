...
export default function AdminAnalyticsPage() {
  ...
  const statusBreakdown = [
    { label: 'Draft',          value: applications.filter(a => a.status === 'DRAFT').length,             color: 'bg-gray-200', text: 'text-gray-600' },
    { label: 'Submitted',      value: applications.filter(a => a.status === 'SUBMITTED').length,          color: 'bg-blue-400', text: 'text-blue-700' },
    { label: 'Under Review',   value: applications.filter(a => a.status === 'UNDER_REVIEW').length,       color: 'bg-yellow-400', text: 'text-yellow-700' },
    { label: 'Missing Docs',   value: applications.filter(a => a.status === 'MISSING_DOCUMENTS').length,  color: 'bg-orange-400', text: 'text-orange-700' },
    { label: 'Approved',       value: approved,                                                            color: 'bg-green-400', text: 'text-green-700' },
    { label: 'Rejected',       value: rejected,                                                            color: 'bg-red-400', text: 'text-red-700' },
  ].filter(s => s.value > 0);

  // By processing tier
  const tierBreakdown = [
    { label: 'Standard',  value: applications.filter(a => a.processingTier === 'STANDARD').length,  color: 'bg-blue-100 text-blue-700' },
    { label: 'Expedited', value: applications.filter(a => a.processingTier === 'EXPEDITED').length, color: 'bg-purple-100 text-purple-700' },
    { label: 'Rush',      value: applications.filter(a => a.processingTier === 'RUSH').length,      color: 'bg-orange-100 text-orange-700' },
  ];

  const summaryCards = [
    { label: 'Total Applications', value: total,        icon: FileText,     color: 'text-blue-600',   bg: 'bg-blue-50',   change: '+12%', up: true },
    { label: 'Approval Rate',      value: `${approvalRate}%`, icon: TrendingUp,  color: 'text-green-600',  bg: 'bg-green-50',  change: '+5%',  up: true },
    { label: 'Pending Review',     value: pending,      icon: Clock,        color: 'text-yellow-600', bg: 'bg-yellow-50', change: '-3%',  up: false },
    { label: 'Rejected',           value: rejected,     icon: XCircle,      color: 'text-red-500',    bg: 'bg-red-50',    change: '-2%',  up: false },
  ];
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-500 mt-1">Application metrics and performance overview.</p>
      </div>
          ...
          <motion.div key={card.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
            <Card>
              <CardContent className="p-5">
                ...
                <div className={`flex items-center gap-1 mt-1 text-xs font-medium ${card.up ? 'text-green-600' : 'text-red-500'}`}>
              ...
              <div className="space-y-2">
                {tierBreakdown.map(t => (
                  <div key={t.label} className="flex items-center gap-3">
                    <span className="text-xs text-gray-500 w-20 flex-shrink-0">{t.label}</span>
        ...
        <Card>
          ...
          <CardContent className="pt-0">
            {topCountries.length === 0 ? (
              <p className="text-gray-400 text-sm text-center py-6">No destination data yet</p>
            ) : (
                  ...
                  <div key={c.name} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs text-gray-500 ml-2 flex-shrink-0">{c.count} apps</span>
                      </div>
