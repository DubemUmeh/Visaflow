  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Review customer tickets and update their status.</p>
      </div>

      {loading ? (
        <div className="flex justify-center py-20"><Loader2 className="h-8 w-8 animate-spin text-coral" /></div>
      ) : tickets.length === 0 ? (
        <Card className="py-16 text-center text-muted-foreground/70">
              ...
              {tickets.map(ticket => (
                <tr key={ticket.id} className="border-b border-border/50">
                  <td className="px-4 py-3">
                    ...
                    <p className="text-xs text-muted-foreground/70">{ticket.ticketNumber}</p>
                  </td>
                  <td className="px-4 py-3 text-xs">{ticket.status.replace(/_/g, ' ')}</td>
                  <td className="px-4 py-3 text-xs">{ticket.priority}</td>
                  <td className="px-4 py-3 text-xs text-muted-foreground">{new Date(ticket.createdAt).toLocaleDateString()}</td>
                  <td className="px-4 py-3">
