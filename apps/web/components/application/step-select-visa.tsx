...
export default function StepSelectVisa() {
  ...
  useEffect(() => {
    ...
    api
      .get(`/visa-types?destinationCountryId=${watchedDest}&nationalityCountryId=${watchedNat}`)
      .then(({ data }) => {
  ...
  const destResults = destSearch
    ? countries.filter(c => c.name.toLowerCase().includes(destSearch.toLowerCase())).slice(0, 8)
    : countries.slice(0, 8);

  const natResults = natSearch
    ? countries.filter(c => c.name.toLowerCase().includes(natSearch.toLowerCase())).slice(0, 8)
    : countries.slice(0, 8);
          ...
          <CardContent className="p-6">
            ...
            {selectedDest ? (
              <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-xl mb-3">
                ...
                <button
                  type="button"
                  onClick={() => { setSelectedDest(null); setValue('destinationCountryId', ''); setValue('visaTypeId', ''); setVisaTypes([]); }}
          ...
          <CardContent className="p-6">
            ...
            {selectedNat ? (
              <div className="flex items-center gap-3 p-3 bg-green-50 rounded-xl mb-3">
                ...
                <button
                  type="button"
                  onClick={() => { setSelectedNat(null); setValue('nationalityCountryId', ''); setValue('visaTypeId', ''); setVisaTypes([]); }}
                  className="text-xs text-blue-600 hover:text-blue-700"
                >Change</button>
              </div>
            ) : (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">
                          {vt.entryType} entry · {vt.stayDuration ? `${vt.stayDuration} day stay` : 'Variable stay'} · {vt.processingDaysMin}–{vt.processingDaysMax} business days
                        </p>
                      </div>
